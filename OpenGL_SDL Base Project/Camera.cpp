...
const float kMoveDistance = 0.5f;
...
void Camera::Render()
{
	...
	glRotatef(m_fYawAngle, 0.0f, 1.0f, 0.0f);
	glRotatef(m_fPitchAngle, 1.0f, 0.0f, 0.0f);
	glRotatef(m_fRollAngle, 0.0f, 0.0f, 1.0f);
...
void Camera::SetAsPerspective(Vector3D aCentrePos, Vector3D aLookAtPos, Vector3D aUpVec, float aNear, float aFar, float aFieldOfView, float aAspect)
{
	...
	m_vRight = Vector3D((m_vFacing.y*m_vUp.z) - (m_vFacing.z*m_vUp.y), (m_vFacing.z*m_vUp.x) - (m_vFacing.x*m_vUp.z), (m_vFacing.x*m_vUp.y) - (m_vFacing.y*m_vUp.x));
...
void Camera::Strafe(float aDistance)
{
	...
	//m_vRight.x += aDistance;

	
}
//go gamescreen1, uncomment camera function
